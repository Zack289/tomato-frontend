...
const RiderOrderMap = ({ order }: props) => {
  const [riderLocaion, setRiderLocaion] = useState<[number, number] | null>(
    null,
  );
    ...
    const fetchLocation = () => {
      navigator.geolocation.getCurrentPosition(
        (pos) => {
          ...
          setRiderLocaion([latitude, longitude]);
  ...
  return (
    <div className="rounded-xl bg-white shadow-sm p-3">
      ...
      >
        ...
        />
        <Marker position={riderLocaion} icon={rideIcon}>
        ...
        <Routing from={riderLocaion} to={deliveryLocation} />
      </MapContainer>
    </div>
  );
};
