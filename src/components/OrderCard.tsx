    ...
    <div className="rounded-xl bg-white p-4 shadow-sm space-y-3">
      ...
      {order.paymentStatus === "paid" && actions.length > 0 && (
        <div className="flex flex-wrap gap-2 pt-2">
          {actions.map((status) => (
            <button
              className="rounded-lg cursor-pointer bg-[#e23744] px-3 py-1 text-xs text-white hover:bg-[#b32f3a] disabled:opacity-50"
              key={status}
              disabled={loading}
      ...
      {order.status === "ready_for_rider" && retryVisible && (
        <div className="pt-2">
          <button
            className="w-full rounded-lg border cursor-pointer border-[#e23744] py-2 text-xs font-semibold text-[#e23744] hover:bg-red-50 disabled:opacity-50"
